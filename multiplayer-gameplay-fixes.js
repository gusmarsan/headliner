(function(){
  ...
  function networkOwnHandHTML(){
    ...
    const cards=owner.deck.slice(0,5),n=cards.length,middle=(n-1)/2,step=n>1?Math.max(58,Math.min(116,Math.round(560/(n-1)))):0;
    return `<div class="player-area"><div class="hand-fan">${cards.map((card,i)=>{const offset=i-middle,ratio=middle?offset/middle:0,x=Math.round(offset*step),angle=Math.round(ratio*11),drop=Math.round(Math.abs(ratio)*12);return `<div class="hand-card" style="--hand-x:${x}px;--hand-x-mobile:${Math.round(x*.52)}px;--hand-rotate:${angle}deg;--hand-drop:${drop}px;--hand-drop-mobile:${Math.round(drop*.55)}px;z-index:${i+2}" onclick="openNetworkOwnDeck()">${liveCardHTML(card,'hand')}</div>`}).join('')}</div></div>`;
  }
